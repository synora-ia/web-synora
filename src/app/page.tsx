...
export default function Home() {
  return (
    <main>
      ...
      <section className="py-32 px-6 bg-white overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-24">
            <h2 className="font-serif-italic text-2xl text-slate-400 mb-4">Metodología Synora</h2>
            <h3 className="text-5xl md:text-6xl font-bold text-black">Del problema a la solución</h3>
          </div>
      ...
      <section id="sectores" className="py-32 px-6 bg-slate-50 border-t border-slate-100">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row items-end justify-between mb-20 gap-8">
            <div className="max-w-2xl">
              <h2 className="font-serif-italic text-2xl text-slate-400 mb-4">Ecosistema de Industrias</h2>
              <h3 className="text-5xl md:text-6xl font-bold text-black tracking-tight">Especialización <br /> <span className="text-slate-400">sectorial total.</span></h3>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="p-8 rounded-[2.5rem] bg-black text-white flex flex-col justify-center items-center text-center">
              <h4 className="text-xl font-bold mb-3">¿Tu sector no aparece?</h4>
              <p className="text-white/40 text-xs mb-8">Diseñamos soluciones personalizadas para cualquier flujo de trabajo.</p>
              <Link href="/contacto" className="px-6 py-3 bg-white text-black rounded-full text-xs font-bold btn-hover">Consultar caso</Link>
            </div>
          </div>
        </div>
      </section>
...
function StatCard({ value, label }: { value: string; label: string }) {
  return (
    <div className="text-center p-8 bg-white rounded-3xl shadow-sm border border-slate-100">
      ...
      <p className="text-sm text-slate-500 uppercase tracking-widest font-semibold">{label}</p>
    </div>
  );
}
...
function Step({ step, title, desc }: { step: string; title: string; desc: string }) {
  return (
    <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/20">
      <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6">Paso {step}</div>
...
function SectorCard({ href, title, desc, icon }: { href: string; title: string; desc: string; icon: React.ReactNode }) {
  return (
    <Link href={href} className="group p-8 rounded-[2rem] bg-white border border-slate-100 hover:border-black transition-all duration-500 hover:shadow-2xl hover:shadow-black/5">
      <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center mb-6 text-slate-400 group-hover:bg-black group-hover:text-white transition-all duration-500">
        {icon}
      </div>
      <h4 className="text-xl font-bold mb-2">{title}</h4>
      <p className="text-slate-500 text-sm mb-6 leading-relaxed">{desc}</p>
      <span className="text-xs font-bold uppercase tracking-widest text-black flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
        Ver solución <svg className="w-3 h-3 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
      </span>
    </Link>
  );
}
...
function ComingSoonCard({ title, desc, icon }: { title: string; desc: string; icon: React.ReactNode }) {
  return (
    <div className="relative group p-8 rounded-[2rem] bg-white border border-slate-100 overflow-hidden">
      <div className="blur-[2px] opacity-40 grayscale pointer-events-none transition-all duration-500">
      ...
      <div className="absolute inset-0 flex items-center justify-center z-20">
        <span className="px-4 py-2 bg-black/5 backdrop-blur-md border border-black/10 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] text-black/40">Próximamente</span>
      </div>
    </div>
  );
}
